...
def train_skipgram(model: SkipGramNeg,
    ...
    for epoch in range(epochs):
        for input_words, target_words in get_batches(words, batch_size, window_size):
            ...
            noise_vectors = model.forward_noise(batch_size=len(inputs), n_samples=window_size)
            ...
            if steps % print_every == 0:
                ...
                # Cosine similarity
                # TODO
                valid_examples, valid_similarities = cosine_similarity(model.in_embed, batch_size, window_size, device=device)
